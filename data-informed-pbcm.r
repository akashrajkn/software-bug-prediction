...
npbootstrap = boot(eclipse_20_required_columns, statFunction, R=500)
values = boot.array(npbootstrap, indices = T)

goodness_of_fit_A = vector(,1000)
goodness_of_fit_B = vector(,1000)


for(j in 1:500)
{
	generated_data <- values[j,]
	...
	goodness_of_fit_A[j] = cross_fit_A_A$bic - cross_fit_A_B$bic
	...
	goodness_of_fit_B[j] = cross_fit_B_A$bic - cross_fit_B_B$bic

}


#fit observed data into models A and B
#obtain the gof

observed_data = eclipse_20_nonnull_rows[, 2]

fitWeibullObs = fitdist(observed_data, "weibull")
fitParetoObs = fitdist(observed_data, "pareto", start = list(shape = 1, scale = 500))

GOF = fitWeibullObs$bic - fitParetoObs$bic


hist(goodness_of_fit_A, freq = F)
lines(density(goodness_of_fit_A))
